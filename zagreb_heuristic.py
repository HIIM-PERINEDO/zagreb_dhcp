...
def infotodict(seqinfo):
    ...
    # DWI
    dwi_ap = create_key('sub-{subject}/{session}/dwi/sub-{subject}_{session}_dir-AP_run-00{item:01d}_dwi')
    ...
    #dwi_pa = create_key('sub-{subject}/{session}/dwi/sub-{subject}_{session}_dir-PA_run-00{item:01d}_dwi')
    
    # fMRI
    rest_ap = create_key('sub-{subject}/{session}/func/sub-{subject}_{session}_task-rest_dir-AP_run-00{item:01d}_bold')
    rest_pa = create_key('sub-{subject}/{session}/func/sub-{subject}_{session}_task-rest_dir-PA_run-00{item:01d}_bold')
    
    # FMAPs
    fmap_se_ap = create_key('sub-{subject}/{session}/fmap/sub-{subject}_{session}_acq-se_dir-AP_run-00{item:01d}_epi')
    fmap_se_pa = create_key('sub-{subject}/{session}/fmap/sub-{subject}_{session}_acq-se_dir-PA_run-00{item:01d}_epi')
    
    # SBRefs
    dwi_ap_sbref = create_key('sub-{subject}/{session}/dwi/sub-{subject}_{session}_dir-AP_run-00{item:01d}_sbref')
    dwi_pa_sbref = create_key('sub-{subject}/{session}/dwi/sub-{subject}_{session}_dir-PA_run-00{item:01d}_sbref')
    rest_ap_sbref = create_key('sub-{subject}/{session}/func/sub-{subject}_{session}_task-rest_dir-AP_run-00{item:01d}_sbref')
    rest_pa_sbref = create_key('sub-{subject}/{session}/func/sub-{subject}_{session}_task-rest_dir-PA_run-00{item:01d}_sbref')

    info = {t1w: [], t2w: [], dwi_ap: [], rest_ap: [], rest_pa: [], fmap_se_ap: [], fmap_se_pa: [], dwi_ap_sbref: [], dwi_pa_sbref: [], rest_ap_sbref: [], rest_pa_sbref: []}
