...
Options:
  ...
  -d / -data-dir  <directory>   The directory used to output the preprocessed files (default: derivatives/dMRI/sub-sID/ses-ssID/dwi/response)
...
# Defaults
datadir=derivatives/dMRI/sub-$sID/ses-$ssID/dwi/response # datadir=derivatives/dMRI_response/sub-$sID/ses-$ssID
...
if [[ $response = tournier ]]; then
    ...
    if [ ! -f ${response}_${dwibase}_response.txt ]; then
	echo "Estimating response function use $response method"
	dwi2response tournier -force -mask  $mask.mif.gz -voxels ${response}_${dwibase}_sf.mif.gz $dwi.mif.gz ${response}_${dwibase}_response.txt
    fi

    echo Check results: response fcn and sf voxels
    echo shview  ${response}_${dwibase}_response.txt
    echo mrview  meanb0_brain.mif.gz -roi.load ${response}_${dwibase}_sf.mif.gz -roi.opacity 0.5 -mode 2
fi
...
if [[ $response = dhollander ]]; then
    ...
    if [ ! -f ${response}_${dwibase}_response.txt ]; then
        ...
        echo "Estimating response function use $response method"
        dwi2response dhollander -force -mask $mask.mif.gz -voxels ${response}_${dwibase}_sf.mif.gz -fa 0.1 $dwi.mif.gz ${response}_${dwibase}_wm.txt ${response}_${dwibase}_gm.txt ${response}_${dwibase}_csf.txt
    fi
    
    echo "Check results for response fcns (wm, gm and csf) and single-fibre voxels (sf)"
    echo shview  $datadir/${response}_${dwibase}_wm.txt
    echo shview  $datadir/${response}_${dwibase}_gm.txt
    echo shview  $datadir/${response}_${dwibase}_csf.txt
    echo mrview  $dwidir/meanb0_brain.mif.gz -overlay.load $datadir/${response}_${dwibase}_sf.mif.gz -overlay.opacity 0.5 -mode 2
...
# shview  $datadir/${response}_${dwibase}_wm.txt
# shview  $datadir/${response}_${dwibase}_gm.txt
# shview  $datadir/${response}_${dwibase}_csf.txt
# mrview  $dwidir/meanb0_brain.mif.gz -overlay.load $datadir/${response}_${dwibase}_sf.mif.gz -overlay.opacity 0.5 -mode 2